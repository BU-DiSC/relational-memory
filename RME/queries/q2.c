...
void run_query2(struct _config_db config_db, struct _config_query params){    
    ...
    if ( config_db.store_type == 'r' ){
        ...
        for (int i = 0; i < config_db.row_count; i++) {
            ...
            if (second_column_value > params.k_value) {
                ...
                cold_array[data_count] = first_column_value;
                data_count++;
            }
        }
        ...
        for (int i = 0; i < config_db.row_count; i++) {
            ...
            if (second_column_value > params.k_value) {
                T first_column_value = *(T*)(plim + i * rme_row_size + params.col_offsets[0]);
                hot_array[data_count] = first_column_value;
                data_count++;
            }
        }
        magic_timing_end(&cycleLo, &cycleHi);
        ...
        for (int i = 0; i < config_db.row_count; i++) {
            ...
            if (column1_value > params.k_value) {
                T column0_value = *(T*)(dram + i * config_db.row_size + params.col_offsets[0]);
                row_array[data_count] = column0_value;
                data_count++;
            }
        }
        magic_timing_end(&cycleLo, &cycleHi);
    ...
    if ( config_db.store_type == 'c' ){
        ...
        for (int i = 0; i < config_db.row_count; i++) {
            ...
            if (column1_value > params.k_value) {
                T column0_value = *(T*)(dram + col0_offset_product + i * sizeof(T));
                col_array[data_count] = column0_value;
                data_count++;
            }
        }
    	magic_timing_end(&cycleLo, &cycleHi);
